...
// models/Feedback.ts  — REPLACE existing
// Fix: FeedbackDocument overrides sentiment field with correct object type
// instead of inheriting `sentiment?: string` from FeedbackType

import mongoose, { Schema, Document, Model } from 'mongoose';
import { Feedback as FeedbackType } from '@/types/feedback';

// Override 'sentiment' — FeedbackType has sentiment?: string (wrong for our schema)
// We redefine it as the proper nested object
export interface FeedbackDocument
  extends Omit<FeedbackType, '_id' | 'sentiment'>,
    Document {
  sentiment?: {
    score:      number;
...
const FeedbackSchema = new Schema<FeedbackDocument>(
  {
    ...
    categories: [{
      type: String,
      enum: [
        'service_quality', 'response_time', 'technician_skill',
        'communication', 'pricing', 'overall_experience',
      ],
    }],
    sentiment: {
      score:      { type: Number },
