...
// Existing GET code same rakho — bas POST add karo
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await Inventory.findOne({ sku: body.sku.toUpperCase(), tenantId: user.tenantId });
    if (existing) return errorResponse('SKU already exists', 400);
