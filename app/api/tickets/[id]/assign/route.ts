...
// app/api/tickets/[id]/assign/route.ts  — REPLACE existing
// Added: NotificationService calls after every assignment action
...
export async function PATCH(
  ...
  try {
    ...
    // ── CASE 1: Route to Service Center ──────────────────────────────────
    if (serviceCenterId && !technicianId) {
      if (user.role === 'service_center') {
        return errorResponse('SC operators must assign to a specific technician', 400);
      }
      ...
      // Fire notification to SC operators of this SC
      const scOperators = await User.find({
        role: 'service_center',
      ...
      if (scOperators.length > 0) {
        await NotificationService.onTicketRoutedToSC({
          scOperatorUserIds: scOperators.map(u => u._id.toString()),
          tenantId: ticket.tenantId,
    ...
    // SC operator: only their own SC's technicians
    if (user.role === 'service_center') {
      const userSCId = request.headers.get('x-service-center-id');
      if (userSCId && tech.serviceCenterId._id.toString() !== userSCId) {
        return errorResponse('Forbidden: technician is not in your service center', 403);
      }
    }
    ...
    if (serviceCenterId) {
      ...
      ticket.serviceCenterId = (tech.serviceCenterId as any)._id;
    }
    ...
    (ticket.timeline as any[]).push({
      action: 'assigned_to_technician',
      description: `Assigned to ${techUserName}${scName ? ` (${scName})` : ''}`,
      performedBy: new Types.ObjectId(user.userId),
      performedByName: performerName,
      metadata: { technicianId, serviceCenterId: (tech.serviceCenterId as any)?._id },
      createdAt: new Date(),
    });

    await ticket.save();

    // Fire notification to the assigned technician
    const techUserId = (tech.userId as any)?._id?.toString();
    if (techUserId) {
      await NotificationService.onTicketAssigned({
