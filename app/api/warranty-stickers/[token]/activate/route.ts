...
export async function POST(
  ...
  try {
    ...
    if (sticker.status === 'activated') {
      return errorResponse(
        'This warranty sticker has already been activated',
        400
      );
    }
    ...
    if (sticker.status === 'expired') {
      return errorResponse(
        'This warranty sticker has expired and cannot be activated',
        400
      );
    }
    ...
    if (body.serialNumber) {
      sticker.serialNumber = body.serialNumber;
    }

    if (body.purchaseDate) {
      sticker.purchaseDate = new Date(body.purchaseDate);
    }

    if (body.customerName) {
      sticker.customerName = body.customerName;
    }

    if (body.customerEmail) {
      sticker.customerEmail = body.customerEmail;
    }

    if (body.customerPhone) {
      sticker.customerPhone = body.customerPhone;
    }
    ...
    const warranty = await Warranty.create({
      ...
      productName: sticker.productName ?? 'Product',
      serialNumber:
        body.serialNumber ?? sticker.serialNumber,
      ...
      warrantyEndDate,
      warrantyType:
        sticker.warrantyType ?? 'standard',
      status: 'active',
      stickerId: sticker._id,
      notes: `Activated via QR code on ${warrantyStartDate.toLocaleDateString(
        'en-IN'
      )}`,
    }).catch(() => null);
